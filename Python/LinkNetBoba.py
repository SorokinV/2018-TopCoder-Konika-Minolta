...
def LinkNetBoba (img_shape, n_out=1, depth=4, 
    ...
    def blockEncoder(i, depth, maxDepth, mm, nn) :
        ...
        io = Conv2D(nn, (3, 3), strides=2, padding='same', name=prefix+'conv1d'+str(maxDepth-depth))(io)
        if batch : io = BatchNormalization(name=prefix+'bath1d'+str(maxDepth-depth))(io)
        io = Activation(acti)(io)
        
        #print('e 1 depth=',depth,io.shape)
        
        io = Conv2D(nn, (3, 3), padding='same', name=prefix+'conv2d'+str(maxDepth-depth))(io)
        if batch : io = BatchNormalization(name=prefix+'bath2d'+str(maxDepth-depth))(io)
        
        ii = Conv2D(nn, (1, 1), strides=2, name=prefix+'ii1d'+str(maxDepth-depth))(i)
        if batch : io = BatchNormalization(name=prefix+'bath3d'+str(maxDepth-depth))(io) # 2017-12-11 add batch
        
        io = Add()([io, ii]); io1 = io;
        
        io = Activation(acti)(io)   # 2017-12-11 change point add before activation after batch
        
        io = Conv2D(nn, (3, 3), padding='same', name=prefix+'conv3d'+str(maxDepth-depth))(io)
        if batch : io = BatchNormalization(name=prefix+'bath4d'+str(maxDepth-depth))(io)
        io = Activation(acti)(io)                
        
        io = Conv2D(nn, (3, 3), padding='same', name=prefix+'conv4d'+str(maxDepth-depth))(io)
        if batch : io = BatchNormalization(name=prefix+'bath5d'+str(maxDepth-depth))(io)
    ...
    def blockDecoder(i, depth, maxDepth, mm, nn) :
        ...
        io = Conv2D(mm//4, (1, 1), padding='same', name=prefix+'convd1d'+str(maxDepth-depth))(io)
        if batch : io = BatchNormalization(name=prefix+'bathd1d'+str(maxDepth-depth))(io)
        io = Activation(acti)(io)                
        
        io = Conv2DTranspose(mm//4, (3, 3), padding='same', name=prefix+'convd2d'+str(maxDepth-depth))(io)
        if batch : io = BatchNormalization(name=prefix+'bathd2d'+str(maxDepth-depth))(io)
        io = Activation(acti)(io)    
        
        io = UpSampling2D((2,2))(io)
        
        io = Conv2D(nn, (1, 1), padding='same', name=prefix+'convd3d'+str(maxDepth-depth))(io)
        if batch : io = BatchNormalization(name=prefix+'bathd3d'+str(maxDepth-depth))(io)
        io = Activation(acti)(io)                
    ...
    i = Input(shape=img_shape, name=prefix+'input'); io = i
    if True : # always exists  
        io = BatchNormalization(name=prefix+'bath0d'+str(maxDepth-depth))(io)
    ...
    io = Conv2D(64, (7, 7), strides=2, padding='same', name=prefix+'conv1d'+str(maxDepth-depth))(io)
    if batch : io = BatchNormalization(name=prefix+'bath1d'+str(maxDepth-depth))(io)
    io = Activation(acti)(io)                
        
    io = MaxPooling2D((3, 3), strides=2, name=prefix+'pool1d'+str(maxDepth-depth))(io)
    ...
    io = Conv2DTranspose(32, (3, 3), padding='same', name=prefix+'conv2d'+str(maxDepth-depth))(io)
    if batch : io = BatchNormalization(name=prefix+'bath2d'+str(maxDepth-depth))(io)
    io = Activation(acti)(io)                
    
    io = UpSampling2D((2,2))(io)
        
    io = Conv2D(32, (3,3), padding='same', name=prefix+'conv3d'+str(maxDepth-depth))(io)
    if batch : io = BatchNormalization(name=prefix+'bath3d'+str(maxDepth-depth))(io)
    io = Activation(acti)(io)                
    
    if dropout : io = SpatialDropout2D(rate=dropout, name=prefix+'dropLd'+str(maxDepth-depth))(io)
    ...
    io = Conv2DTranspose(n_out, (2, 2), strides=2, padding='same', name=prefix+'conv4d'+str(maxDepth-depth))(io)
    if batch : io = BatchNormalization(name=prefix+'bath4d'+str(maxDepth-depth))(io)
    io = Activation(acti)(io)                

    io = Conv2D(n_out, (3,3), padding='same', name=prefix+'conv5d'+str(maxDepth-depth))(io)
    if batch : io = BatchNormalization(name=prefix+'bath5d'+str(maxDepth-depth))(io)
    io = Activation(acti)(io)

    io = Conv2D(n_out, (1,1), padding='same', name=prefix+'convLd'+str(maxDepth-depth))(io)
